...
class PlayState extends MusicBeatState
{
	...
	public var isClashsterfunk:Bool = false;
	...
	override public function create()
	{
		...
		isClashsterfunk = (SONG.song.toLowerCase() == 'clashsterfunk' || SONG.song.toLowerCase() == 'bipolar');
		crazyMode = (SONG.song.toLowerCase() == 'clashsterfunk' || SONG.song.toLowerCase() == 'bipolar');
		if (crazyMode)
		{
			health = 2;
		}
		...
		switch (songLowercase)
		{
			...
			case 'cultivate':
				dialogue = CoolUtil.coolTextFile(Paths.txt('cultivate/cultivateDialogue'));
			case 'conflict':
				dialogue = CoolUtil.coolTextFile(Paths.txt('conflict/conflictDialogue'));
			case 'clashsterfunk':
				dialogue = CoolUtil.coolTextFile(Paths.txt('clashsterfunk/clashsterfunkDialogue'));
		}
		...
		switch(stageCheck)
		{
			...
			case 'split':
			{
					...
					var tex = Paths.getSparrowAtlas('paper/splitbg','week7');
					trace(tex);

					var spl = new FlxSprite(-600, -100);
					spl.frames = tex;
					spl.animation.addByPrefix('idle', 'background 2', 5);
					spl.animation.play('idle');
					spl.scale.set(0.675, 0.675);
					spl.updateHitbox();
					add(spl);

					var tex = Paths.getSparrowAtlas('paper/splitStatic','week7');
					trace(tex);

					splst = new FlxSprite(-600, -100);
					splst.frames = tex;
					splst.animation.addByPrefix('idle', 'background 2', 15);
					splst.animation.play('idle');
					splst.scale.set(0.675, 0.675);
					splst.updateHitbox();
					add(splst);
					splst.visible = false;
			}
			case 'splitX':
			{
					...
					var tex = Paths.getSparrowAtlas('paper/splitX','week7');
					trace(tex);
					...
					var tex = Paths.getSparrowAtlas('paper/splitStaticX','week7');
					trace(tex);
			...
			{
					defaultCamZoom = 1;
					curStage = 'splitStatic';

					var tex = Paths.getSparrowAtlas('paper/splitStatic','week7');
					trace(tex);

					var bg = new FlxSprite(-600, -100);
					bg.frames = tex;
					bg.animation.addByPrefix('idle', 'background 2', 15);
					bg.animation.play('idle');
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'conflict' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'clashsterfunk')
		{
			remove(black);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'normalX':
					babyArrow.frames = Paths.getSparrowAtlas('NOTE_assetsX');
					babyArrow.animation.addByPrefix('green', 'arrow static instance 1');
					babyArrow.animation.addByPrefix('blue', 'arrow static instance 2');
					babyArrow.animation.addByPrefix('purple', 'arrow static instance 3');
					babyArrow.animation.addByPrefix('red', 'arrow static instance 4');
	...
	*/
	...
	override function stepHit()
	{
		...
		// if (curSong.toLowerCase() == 'clashsterfunk') 
		// {
		// 		if (curStep == 2 || curStep == 40 || curStep == 59)
		// 			splst.visible = true;
		// 		else if (curStep == 25 || curStep == 45 || curStep == 62)
		// 			splst.visible = false;
		// 		else if (curStep > 62 && curStep % 20 == 0)
		// 			if (splst.visible)
		// 				splst.visible =  false;
		// 			else
		// 				splst.visible =  true;
		...
		if (curSong.toLowerCase() == 'conflict') 
		{
				if (curStep == 1216)
				{
					remove(dad);
					dad = new Character(-350, 40, 'coby');
					add(dad);
				}
		}
